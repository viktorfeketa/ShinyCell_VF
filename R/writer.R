...
wrSVfix <- function() {
  glue::glue('### Useful stuff \n',
             ...
             '# VF\n',
             '# 5/8/2024\n',
             '# function to produce table with statistics for the current violin view\n',
             '# based on Plot violin / boxplot \n',
             'vfGenStatsTable <- function(inpConf, inpMeta, inp1, inp2, \n',
             '                     inpsub1, inpsub2, inpH5, inpGene, \n',
             '                     inptyp, inppts, inpsiz, inpfsz){{ \n',
             '  if(is.null(inpsub1)){{inpsub1 = inpConf$UI[1]}} \n',
             '  # Prepare ggData \n',
             ...
             '  # perform stats\n',
             '  \n',
             '  # (deprecated) get individual group data\n',
             '  # data.group1 <- ggData %>% filter(X == "Monomer") %>% dplyr::select(val) %>% pull\n',
             '  # data.group2 <- ggData %>% filter(X == "PFF") %>% dplyr::select(val) %>% pull\n',
             '  # stat.result <- wilcox.test(data.group1, data.group2)\n',
             '  \n',
             '  # run stat test based on grouping variable\n',
             '  stat.result <- wilcox.test(val ~ X, data = ggData, exact = FALSE)\n',
             '  p.val <- stat.result$p.value\n',
             '  \n',
             '  # format p.val to human friendly\n',
             '  p.val.format <- format.pval(p.val, digits = 3, 0.001)\n',
             '  \n',
             '  # calculate summary data\n',
             '  data.sum <- ggData %>% group_by(X) %>% summarise(count = n(), median = median(val), mean = mean(val))\n',
             '  \n',
             '  # add p-value to summary data\n',
             '  data.sum$p.value <- c("", p.val.format)\n',
             '  \n',
             '  # re-name grouping column\n',
             '  colnames(data.sum)[1] <- inp1\n',
             '  \n',
             '  return(data.sum)\n',
             '}} \n',
             '\n',
             ...
             '# VF\n',
             '# 5/8/2024\n',
             '# function to produce table with top DE genes\n',
             '# based on Plot violin / boxplot \n',
             'vfGenTopDEGenes <- function(inpConf, inpMeta, inp1, inp2, \n',
             '                            inpsub1, inpsub2, inpH5, inpGene, \n',
             '                            inptyp, inppts, inpsiz, inpfsz, expr.data, groupings){{ \n',
             '  \n',
             '  # settings\n',
             '  n.top.genes <- 20\n',
             '  \n',
             ...
             '\n',
             '  if (is.null(inpsub2)){{\n',
             '    # all.cells; do nothing; give user a tip to select subset\n',
             '    test.result.formatted <- data.frame("tip" = "Select some subset of cells to run the DE test. Be patient - DE test may take up to 30 sec to run.")\n',
             '    \n',
             ' \n',
             '    # alternatively, run test on all cells, but takes too long, about 20 sec\n',
             '    \n',
             '    # test.result <- wilcoxauc(expr.data[1:100,], groupings)\n',
             '    # top.markers <- top_markers(test.result)\n',
             '    # \n',
             '    # test.result$FoldChange <- 2^test.result$logFC\n',
             '    # test.result.formatted <- test.result %>%\n',
             '    #   filter(group=="PFF") %>%\n',
             '    #   arrange(desc(logFC)) %>%\n',
             '    #   select(feature, avgExpr, FoldChange, pval, padj, pct_in, pct_out) %>%\n',
             '    #   slice_head(n=n.top.genes)\n',
             '    # \n',
             '    # return(test.result.formatted)\n',
             '    \n',
             '  }} else {{\n',
             '    # some subset is selected\n',
             '    \n',
             '    # protection against testing the whole matrix\n',
             '\n',
             '    subset.col.name <- inpConf[UI == inpsub1]$ID\n',
             '    cells.to.use <- subset(inpMeta, subset = (get(subset.col.name) %in% inpsub2)) %>% select(sampleID) %>% pull\n',
             '    cells.to.use.indices <- match(cells.to.use, inpMeta$sampleID)\n',
             '    \n',
             '    # if selection of subset results in testing all the cells, then dont perform test\n',
             '    # it is too computationally expensive\n',
             '    if (length(cells.to.use) == ncol(expr.data)) {{\n',
             '      # dont test\n',
             '      # create a fake data frame with message\n',
             '      test.result.formatted <- data.frame("error" = "Current subset parameters result in selecting all cells. The DE test will not be performed. Please select only some subset of cells.")\n',
             '    }} else {{\n',
             '      filtered.matrix <- expr.data[, cells.to.use.indices]\n',
             '      filtered.groupings <- groupings[cells.to.use.indices]\n',
             '      row.names(filtered.matrix) <- names(sort(inpGene))\n',
             '      \n',
             '      # this call uses more than 8GB of memory, which crashes shinyapps\n',
             '      test.result <- wilcoxauc(filtered.matrix, filtered.groupings)\n',
             '      \n',
             '      # alternative:\n',
             '      # split in chunks of 5000 genes, run several times, combine outputs\n',
             '      \n',
             '      # # run wilcox test on chunks of N genes at at time in a loop\n',
             '      # # instead of the whole matrix in one call\n',
             '      # num.genes <- nrow(filtered.matrix)\n',
             '      # test.result <- data.frame()\n',
             '      # i.step <- 5000\n',
             '      # for (i.start.gene in seq(1, num.genes, by=i.step)){{\n',
             '      #   i.start.row <- i.start.gene\n',
             '      #   i.end.row <- i.start.gene + i.step - 1\n',
             '      #   if (i.end.row > num.genes) {{i.end.row <- num.genes}}\n',
             '      #   test.result.step <- wilcoxauc(filtered.matrix[(i.start.row):(i.end.row),], filtered.groupings)\n',
             '      #   test.result <- rbind(test.result, test.result.step)\n',
             '      #   test.result.step <- NULL\n',
             '      #   gc()\n',
             '      # }}\n',
             '      \n',
             '      #top.markers <- top_markers(test.result)\n',
             '      \n',
             '      # get group names for the grouping variable from configuration data frame\n',
             '      group.names <- strsplit(inpConf[UI == "Exp_Group"]$fID, "|", fixed = TRUE)[[1]]\n',
             '      \n',
             '      test.result.g1 <- test.result %>% filter(group==group.names[1])\n',
             '      test.result.g2 <- test.result %>% filter(group==group.names[2])\n',
             '      \n',
             '      test.result.g2["FoldChange"] <- 2^test.result.g2$logFC\n',
             '      test.result.g2[paste0(group.names[1], "_Avg_Expr")] <- test.result.g1$avgExpr\n',
             '      test.result.g2[paste0(group.names[1], "_Pct_In")] <- test.result.g1$pct_in\n',
             '      test.result.g2[paste0(group.names[1], "_Pct_Out")] <- test.result.g1$pct_out\n',
             '      test.result.g2[paste0(group.names[2], "_Avg_Expr")] <- test.result.g2$avgExpr\n',
             '      test.result.g2[paste0(group.names[2], "_Pct_In")] <- test.result.g2$pct_in\n',
             '      test.result.g2[paste0(group.names[2], "_Pct_Out")] <- test.result.g2$pct_out\n',
             '      test.result.g2$Pval <- test.result.g2$pval\n',
             '      test.result.g2$Padj <- test.result.g2$padj\n',
             '      \n',
             '      test.result.formatted <- test.result.g2 %>%\n',
             '        arrange(desc(logFC)) %>%\n',
             '        select(feature, FoldChange, Pval, Padj,\n',
             '               paste0(group.names[1], "_Avg_Expr"),\n',
             '               paste0(group.names[1], "_Pct_In"),\n',
             '               paste0(group.names[1], "_Pct_Out"),\n',
             '               paste0(group.names[2], "_Avg_Expr"),\n',
             '               paste0(group.names[2], "_Pct_In"),\n',
             '               paste0(group.names[2], "_Pct_Out")) %>%\n',
             '        slice_head(n=n.top.genes)\n',
             '      \n',
             '      # test.result.pff$FoldChange <- 2^test.result.pff$logFC\n',
             '      # test.result.pff$Ctrl_Avg_Expr <- test.result.ctrl$avgExpr\n',
             '      # test.result.pff$Ctrl_Pct_In <- test.result.ctrl$pct_in\n',
             '      # test.result.pff$Ctrl_Pct_Out <- test.result.ctrl$pct_out\n',
             '      # test.result.pff$PFF_Avg_Expr <- test.result.pff$avgExpr\n',
             '      # test.result.pff$PFF_Pct_In <- test.result.pff$pct_in\n',
             '      # test.result.pff$PFF_Pct_Out <- test.result.pff$pct_out\n',
             '      # test.result.pff$Pval <- test.result.pff$pval\n',
             '      # test.result.pff$Padj <- test.result.pff$padj\n',
             '      # \n',
             '      # test.result.formatted <- test.result.pff %>%\n',
             '      #   arrange(desc(logFC)) %>%\n',
             '      #   select(feature, FoldChange, Pval, Padj, Ctrl_Avg_Expr, Ctrl_Pct_In, Ctrl_Pct_Out, PFF_Avg_Expr, PFF_Pct_In, PFF_Pct_Out) %>%\n',
             '      #   slice_head(n=n.top.genes)\n',
             '  \n',
             '    }}\n',
             '    \n',
             ...
             ' \n',
             ' \n',
             '  # Enable assignment of reactive variable that stores\n',
             '  # dataset-specific .expr.data when user navigates to respective page\n',
             '  # Only load the variable from file if not yet loaded\n',
             '  # Observe when the user navigates to a different tab\n',
             '  observeEvent(input$navbar, {{\n',
             '    if (grepl("_de_tab", input$navbar)) {{\n',
             '		# get variable prefix\n',
             '		var.prefix <- gsub("_de_tab", "", input$navbar)\n',
             '		reactive.var.name <- paste0(var.prefix, ".expr.data")\n',
             '		\n',
             '		reactive.var.null <- is.null(get(reactive.var.name)())\n',
             '		\n',
             '		# message(reactive.var.name)\n',
             '		# message(reactive.var.null)\n',
             '		\n',
             '		if (reactive.var.null) {{\n',
             '			message(paste0("Loading data for ", reactive.var.name))\n',
             '			expr.data.load <- qread(file = paste0(var.prefix, "_srInt_data_matrix.qs"))\n',
             '			get(reactive.var.name)(expr.data.load)\n',
             '			\n',
             '			reactive.var.name.groupings <- sub(".expr.data", ".groupings", reactive.var.name)\n',
             '			groupings.data.load <- qread(file = paste0(var.prefix, "_srInt_groupings.qs"))\n',
             '			get(reactive.var.name.groupings)(groupings.data.load)\n',
             '		}} else {{\n',
             '			message(paste0("Data already loaded for ", reactive.var.name))\n',
             '		}}\n',
             '	}}\n',
             '   \n',
             '  }})\n',
...
#' @export wrSVmain
#'
wrSVmain <- function(prefix, grouping.var = "group", subst = "") {
  glue::glue('optCrt="{{ option_create: function(data,escape) {{return(\'<div class=\\"create\\"><strong>\' + \'</strong></div>\');}} }}" \n', 
             ...
             '  }}) \n',
             '  output${prefix}violin.stats.table <- renderTable({{\n',
             '\n',
             '    vfGenStatsTable({prefix}conf, {prefix}meta, input${prefix}c1inp1, input${prefix}c1inp2, \n',
             '                    input${prefix}c1sub1, input${prefix}c1sub2, \n',
             '                    "{prefix}gexpr.h5", {prefix}gene, input${prefix}c1typ, input${prefix}c1pts, \n',
             '                    input${prefix}c1siz, input${prefix}c1fsz)\n',
             '  }},\n',
             ...
             '      HTML("<hr>"),\n',
             '      if (input${prefix}c1inp1 == "{grouping.var}") {{\n',
             '        h4("Statistics")\n',
             '      }},\n',
             '      if (input${prefix}c1inp1 == "{grouping.var}") {{\n',
             '        tableOutput("{prefix}violin.stats.table")\n',
             '      }},\n',
             '      if (input${prefix}c1inp1 == "{grouping.var}") {{\n',
             '        HTML("<hr>")\n',
             '      }}\n',
             ...
             '  }}) \n',
             '   \n',
             '  ### Plots for tab DE genes \n',
             '  output${prefix}c1sub1vf1.ui <- renderUI({{ \n',
             '    sub = strsplit({prefix}conf[UI == input${prefix}c1sub1vf1]$fID, "\\\\|")[[1]] \n',
             '    checkboxGroupInput("{prefix}c1sub2vf1", "Select which cells to show", inline = TRUE, \n',
             '                       choices = sub, selected = sub) \n',
             '  }}) \n',
             '  observeEvent(input${prefix}c1sub1nonvf1, {{ \n',
             '    sub = strsplit({prefix}conf[UI == input${prefix}c1sub1vf1]$fID, "\\\\|")[[1]] \n',
             '    updateCheckboxGroupInput(session, inputId = "{prefix}c1sub2vf1", label = "Select which cells to show", \n',
             '                             choices = sub, selected = NULL, inline = TRUE) \n',
             '  }}) \n',
             '  observeEvent(input${prefix}c1sub1allvf1, {{ \n',
             '    sub = strsplit({prefix}conf[UI == input${prefix}c1sub1vf1]$fID, "\\\\|")[[1]] \n',
             '    updateCheckboxGroupInput(session, inputId = "{prefix}c1sub2vf1", label = "Select which cells to show", \n',
             '                             choices = sub, selected = sub, inline = TRUE) \n',
             '  }}) \n',
             '\n',
             '   \n',
             '  output${prefix}de.table <- renderTable({{\n',
             '    \n',
             '    vfGenTopDEGenes({prefix}conf, {prefix}meta, input${prefix}c1inp1vf1, NULL, \n',
             '                    input${prefix}c1sub1vf1, input${prefix}c1sub2vf1, \n',
             '                    "{prefix}gexpr.h5", {prefix}gene, input${prefix}c1typ, input${prefix}c1pts, \n',
             '                    input${prefix}c1siz, input${prefix}c1fsz, {prefix}.expr.data(), {prefix}.groupings())\n',
             '  }},\n',
             '  digits = 4)\n',
             '  \n',
             '  output${prefix}c1oupvf1.ui <- renderUI({{ \n',
             '    \n',
             '    tags$div(\n',
             '      h5("Top DE genes between Control and PFF in selected subset of cells"),\n',
             '      tableOutput("{prefix}de.table"),\n',
             '      HTML("<hr>"))\n',
             '      \n',
             '    \n',
             '  }})\n',
             ...
             '  \n',
             '  \n',
             '  # Reactive value to store the variable\n',
             '  # Dataset-specific .expr.data variable\n',
             '  {prefix}.expr.data <- reactiveVal(NULL)\n',
             '  {prefix}.groupings <- reactiveVal(NULL)\n',
             '  \n',
             '  \n',
              '         \n')
}
...
#' @export wrUImain
#'
wrUImain <- function(prefix, grouping.var = "group", subst = "", ptsiz = "1.25") {
  glue::glue('  ### Tab1.a1: cellInfo vs geneExpr on dimRed \n',
             ...
             '   \n',
             '### VF 5/9/2024\n',
             '### New tab: top DE genes\n',
             '### based on:\n',
             '### Tab1.c1: violinplot / boxplot \n',
             'tabPanel(title = "Top DE Genes", value = "{prefix}_de_tab", \n',
             '  h4("Top DE Genes"), \n',
             ...
             '    column( \n',
             '      3, style="border-right: 2px solid black", \n',
             '      selectInput("{prefix}c1inp1vf1", "Grouping Variable:", \n',
             '                  choices = c("{grouping.var}"), \n',
             '                  selected = c("{grouping.var}")) %>%  \n',
             '        helper(type = "inline", size = "m", fade = TRUE, \n',
             '               title = "Cell information to group cells by",  \n',
             '               content = c("Select categorical cell information to group cells by",  \n',
             '                           "- Single cells are grouped by this categorical covariate",  \n',
             '                           "- Plotted as the X-axis of the violin plot / box plot")),  \n',
             '      actionButton("{prefix}c1togLvf1", "Toggle to subset cells"), \n',
             '      conditionalPanel( \n',
             '        condition = "input.{prefix}c1togLvf1 % 2 == 1", \n',
             '        selectInput("{prefix}c1sub1vf1", "Cell information to subset:", \n',
             '                    choices = {prefix}conf[grp == TRUE]$UI, \n',
             '                    selected = {prefix}def$grp1), \n',
             '        uiOutput("{prefix}c1sub1vf1.ui"), \n',
             '        actionButton("{prefix}c1sub1allvf1", "Select all groups", class = "btn btn-primary"), \n',
             '        actionButton("{prefix}c1sub1nonvf1", "Deselect all groups", class = "btn btn-primary") \n',
             '      ), br(), br(), \n',
             '       \n',
             '    ), # End of column (6 space) \n',
             '    column(9, uiOutput("{prefix}c1oupvf1.ui")  \n',
             '\n',
             '    )  # End of column (6 space) \n',
             '  )    # End of fluidRow (4 space) \n',
             ')     # End of tab (2 space) \n',
...
#' @export wrUIend
#'
wrUIend <- function(footnote, version) {
  if(is.list(footnote)){
  ...
  glue::glue('   \n',
             ...
             'p({f0}), \n',
             'p("Version: {version}"), \n',
             'p(em("This webpage was made by Viktor Feketa ("), \n',
              ' a("viktor.feketa@yale.edu", \n',
              '   href = "mailto: viktor.feketa@yale.edu"), \n',
              ' em(") using "), \n',
              ' a("Shiny",  \n',
              '   href = "https://www.rstudio.com/products/shiny/",target="_blank"), \n',
              ' em(" and "), \n',
              ' a("ShinyCell", \n',
              '   href = "https://github.com/SGDDNB/ShinyCell",target="_blank"), \n',
              ' em(" with modifications.") \n',
             '),  \n',
             'br(),br(),br(),br(),br() \n',
             ') \n',
             '))) \n',
